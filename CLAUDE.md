...
- `App.xaml.cs` is the composition root: builds the five providers → aggregator → view model → windows → tray icon, loads/saves `AppSettings` on startup/exit, and owns the `DispatcherTimer`. `App.xaml` sets `ShutdownMode="OnExplicitShutdown"` so hiding the widget or closing the popup doesn't quit the app — only the tray "종료" menu item calls `Shutdown()`. It also registers `DispatcherUnhandledException`/`AppDomain.UnhandledException` handlers that log to `%AppData%\SysMonWidget\crash.log` and (for UI-thread exceptions) mark the exception handled so the widget keeps running instead of silently dying — see gotchas below.
...
## Metrics collection reliability gotchas (already worked around — watch for regressions)

- **`DispatcherTimer` doesn't wait for the previous async `Tick` to finish.** Since collection runs via `await Task.Run(...)` inside the `Tick` handler, a slow collection (many GPU/NIC instances) can still be running when the next 1s tick fires, launching a second overlapping collection. `Cpu`/`DiskMetricsProvider` reuse a single long-lived `PerformanceCounter` field across calls, and `PerformanceCounter` isn't thread-safe for concurrent `NextValue()` — overlap corrupts the delta calculation (observed as CPU reading pinned at 100%). `App.xaml.cs` guards this with an `_isCollecting` flag that skips a tick if the previous one hasn't finished.
- **Rate counters (e.g. `Bytes Total/sec`) always return 0 on their first `NextValue()` call** — they're computed from the delta between two samples, so a freshly-constructed counter needs a throwaway warm-up call before the real read. `Cpu`/`Disk` providers get this for free because they keep one counter alive for the app's lifetime, but `NetworkMetricsProvider` recreates counters every call and initially only sampled once, so NET stayed at 0% forever. Any new provider that constructs a fresh rate-type counter per call must warm it up first (see `NetworkMetricsProvider.GetCurrentValue`).
- **Per-process performance counter instances (e.g. `GPU Engine`) can disappear mid-read.** If the process behind an instance exits between enumerating instance names and reading the counter, `PerformanceCounter` throws `InvalidOperationException`. With no exception handling anywhere in the app, this silently killed the whole widget at unpredictable intervals. `GpuMetricsProvider` wraps counter construction and each `NextValue()` call to skip a vanished instance instead of throwing — apply the same pattern to any other provider that enumerates a dynamic, per-process counter category. The `App.xaml.cs` unhandled-exception handlers (above) are the last line of defense either way.
- Don't expect exact parity with Task Manager's numbers. Since Windows 8, Task Manager's disk graph is computed from ETW, not the classic `% Disk Time` counter this app uses — a small, stable offset (a few percent) is expected and isn't a bug.
